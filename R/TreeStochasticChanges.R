...
#' StateChanges <-
#'   TreeStochasticChanges(CladisticMatrix = CladisticMatrix,
...
#'
#' @export TreeStochasticChanges
TreeStochasticChanges <- function(CladisticMatrix, Tree, TimeBins, NSimulations = 10, PolymorphismBehaviour = "equalp", UncertaintyBehaviour = "equalp", InapplicableBehaviour = "missing") {
  ...
  # ADD INAPPLICABLE OPTION THAT TIES SWITCH TO "" TO DEPENDENT CHARACTER A LA MatrixDistances APPROACH.
  ...
  # Get tree node ages:
  TreeTreeNodeAges <- TreeNodeAges(Tree)
  ...
  PruneTree <- function(x) {
    ...
    if(length(TipsToDrop) > 0) {
      ...
      if((TipsInTree - length(TipsToDrop)) == 1) {
        ...
        x$PrunedTree$root.time <- unname(TreeTreeNodeAges[Tree$edge[TipEdge, 1]])
      ...
      if((TipsInTree - length(TipsToDrop)) == 2) {
        ...
        x$PrunedTree$root.time <- unname(TreeTreeNodeAges[TreeMostRecentCommonAncestor(setdiff(Tree$tip.label, TipsToDrop), Tree)])
      ...
      if((TipsInTree - length(TipsToDrop)) > 2) {
        ...
        x$PrunedTree <- TreeFixRootTime(Tree, x$PrunedTree)
  ...
  GetPrunedTreeNodeAges <- function(x) {
    ...
    if(NumberOfTips == 1) {
      ...
      x$PrunedTreeNodeAges <- c(x$PrunedTree$root.time - sum(x$PrunedTree$edge.length), x$PrunedTree$root.time)
      
      # Set node numbers as 1 for the tip and 2 for the node subtending the sole branch:
      names(x$PrunedTreeNodeAges) <- as.character(1:2)
    ...
    if(NumberOfTips == 2) {
      ...
      x$PrunedTreeNodeAges <- c(x$PrunedTree$root.time - x$PrunedTree$edge.length[order(x$PrunedTree$edge[, 2])], x$PrunedTree$root.time)
      
      # Set node ages as 1 and 2 (for tips) and 3 for root:
      names(x$PrunedTreeNodeAges) <- as.character(1:3)
      
    }
    
    # If more than two tips just apply get node ages function:
    if(NumberOfTips > 2) x$PrunedTreeNodeAges <- TreeNodeAges(x$PrunedTree)
  ...
  CharacterList <- lapply(CharacterList, GetPrunedTreeNodeAges)
  ...
  PerformEdgeMatches <- function(x, tree) {
    ...
    if(NumberOfTips == 2) {
      ...
      AncestorNode <- TreeMostRecentCommonAncestor(descs = TipNames, tree = tree)
    ...
    if(NumberOfTips > 2) x$PrunedToFullTreeEdgeMatches <- TreeMatchEdges(Tree, x$PrunedTree)$matching.edges
        ...
        for(i in 1:length(y)) {
          ...
          if(length(MultipleEdgeMultipleStateMatches) > 0) {
            ...
            for(j in MultipleEdgeMultipleStateMatches) {
              ...
              StartAgeOfPrunedEdge <- unname(x$PrunedTreeNodeAges[x$PrunedTree$edge[j, 1]])
  ...
  ReformatCharacterChanges <- function(x) {
    ...
    ExtractCharacterChanges <- function(y) {
      ...
      if(length(EdgesWithChanges) > 0) {
        ...
        AgeAtStartOfEdgesWithChanges <- TreeTreeNodeAges[x$FullTree$edge[EdgesWithChanges, 1]]
      ...
      for(i in PrecedingChanges[!duplicated(Tree$edge[PrecedingChanges, 1])]) {
        ...
        # If some other state:
          ...
          ChangesMatrix <- rbind(ChangesMatrix, c(NA, EdgeFromTo[i, 1], which(Tree$edge[, 2] == Tree$edge[i, 1]), unname(TreeTreeNodeAges[Tree$edge[i, 1]])))
      ...
      if(length(FollowingChanges) > 0) {
        ...
        ChangesMatrix <- rbind(ChangesMatrix, cbind(EdgeFromTo[FollowingChanges, 2], FollowingState[FollowingChanges], unlist(lapply(as.list(Tree$edge[FollowingChanges, 2]), function(z) {FollowingEdges <- which(Tree$edge[, 1] == z); FollowingEdges[is.na(EdgeFromTo[FollowingEdges, 1])]})), unname(TreeTreeNodeAges[Tree$edge[FollowingChanges, 2]])))
