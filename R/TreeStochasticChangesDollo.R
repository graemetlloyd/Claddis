...
#' out <- TreeStochasticChangesDollo(tree, tip.states)
#'
...
#' out$SCM
#'
#' @export TreeStochasticChangesDollo
TreeStochasticChangesDollo <- function(tree, tip.states) {
  ...
  new.root <- TreeMostRecentCommonAncestor(names(which(tip.states == 1)), tree)
  ...
  # If new root reflects a subtree:
    ...
    if(sum(tip.states == 1) == 1) {
      ...
      acquisition.bounds <- TreeNodeAges(tree)[tree$edge[acquisition.branch, ]]
    ...
    # Case if at least two taxa exhibit the derived state:
      ...
      if(length(clade.members) == 2) {
        ...
        descendant.edges <- TreeDescendantEdges(new.root, tree)
      ...
      # Case if at least three members in clade:
        ...
        new.tree <- TreeFixRootTime(tree, new.tree)
  ...
  all.node.ages <- TreeNodeAges(tree)
