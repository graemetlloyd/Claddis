...
#' @param EstimateAllNodes Option passed to internal use of \link{EstimateAncestralStates}.
#' @param EstimateTipValues Option passed to internal use of \link{EstimateAncestralStates}.
#' @param InapplicablesAsMissing Option passed to internal use of \link{EstimateAncestralStates}.
#' @param PolymorphismBehaviour Option passed to internal use of \link{EstimateAncestralStates}.
#' @param UncertaintyBehaviour Option passed to internal use of \link{EstimateAncestralStates}.
#' @param Threshold Option passed to internal use of \link{EstimateAncestralStates}.
#'
#' @details
#'
#' \bold{Introduction}
#'
#' Morphological change can be captured by discrete characters and their evolution modelled as occurring along the branches of a phylogenetic tree. This function takes as primary input a character-taxon matrix of discrete characters (in the format imported by \link{ReadMatrixNEXUS}) and a time-scaled phylogenetic tree (in the format of \pkg{paleotree} or \pkg{strap}) and begins by inferring ancestral states at the tree's internal nodes using the \link{EstimateAncestralStates} function. From here changes along individual branches can be estimated (only the minimum number of changes are inferred; see \link{GetStochasticChanges} for an alternative but unfinished approach) and hence rates can be calculated.
#'
...
#'
#' Finally, the remaining options (\code{EstimateAllNodes}, \code{EstimateTipValues}, \code{InapplicablesAsMissing}, \code{PolymorphismBehaviour}, \code{UncertaintyBehaviour}, and \code{Threshold}) are all simply passed directly to \link{EstimateAncestralStates} for estimating the ancestral states and users should consult the help file for that function for further details.
#'
...
DiscreteCharacterRate <- function(tree, CladisticMatrix, TimeBins, BranchPartitionsToTest = NULL, CharacterPartitionsToTest = NULL, CladePartitionsToTest = NULL, TimeBinPartitionsToTest = NULL, ChangeTimes = "random", LikelihoodTest = "AIC", Alpha = 0.01, MultipleComparisonCorrection = "BenjaminiHochberg", PolymorphismState = "missing", UncertaintyState = "missing", InapplicableState = "missing", TimeBinApproach = "Lloyd", EnsureAllWeightsAreIntegers = FALSE, EstimateAllNodes = FALSE, EstimateTipValues = FALSE, InapplicablesAsMissing = FALSE, PolymorphismBehaviour = "equalp", UncertaintyBehaviour = "equalp", Threshold = 0.01) {
  ...
  AncestralStates <- EstimateAncestralStates(CladisticMatrix = CladisticMatrix, Tree = tree, EstimateAllNodes = EstimateAllNodes, EstimateTipValues = EstimateTipValues, InapplicablesAsMissing = InapplicablesAsMissing, PolymorphismBehaviour = PolymorphismBehaviour, UncertaintyBehaviour = UncertaintyBehaviour, Threshold = Threshold)
