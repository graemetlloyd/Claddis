...
GetAllStateChanges <- function(CladisticMatrix, tree, time.bins, Nsim = 10) {
  ...
  DataTypes <- unname(do.call(c, lapply(CladisticMatrix[2:length(CladisticMatrix)], function(x) rep(x[["Datatype"]], ncol(x[["Matrix"]])))))
  ...
  char.model.sets <- paste(unlist(lapply(lapply(lapply(lapply(apply(CladisticMatrix$Matrix_1$Matrix, 2, strsplit, split = "&"), unlist), sort), unique), paste, collapse = "")), CladisticMatrix$Matrix_1$Ordering, sep = "")
  ...
  for(i in unique(char.model.sets)) {
    ...
    if(length(states) > 1) {
      ...
      # Check for confounding characters (those which are unordered, but not all states between the minimum and the maximum are sampled) stop and give the user an error message:
      if((diff(range(states)) + 1) != length(states) && ordering.type == "ord") stop(paste("ERROR: cannot currently deal with the following ordered character(s) where not all states are sampled amongst tips: ", paste(which(char.model.sets == i), collapse = " "), sep = ""))
      
      # If character is unordered make all off-diagonal elements equal to 1:
      if(ordering.type == "unord") for(j in 1:nrow(char.model)) char.model[setdiff(1:nrow(char.model), j), j] <- 1
      
      # If character is ordered make only immediate off-diagonal elements equal to 1:
      if(ordering.type == "ord") for(j in 1:nrow(char.model)) char.model[setdiff(1:nrow(char.model), j)[abs(setdiff(1:nrow(char.model), j) - j) == 1], j] <- 1
  ...
  chartime <- rep(0, sum(unlist(lapply(lapply(CladisticMatrix[2:length(CladisticMatrix)], '[[', "Matrix"), ncol))))
  
  # Matrix to store time for each character:
  terminal.edge.lengths.per.bin <- internal.edge.lengths.per.bin <- edge.lengths.per.bin <- matrix(0, ncol = length(time.bins) - 1, nrow = sum(unlist(lapply(lapply(CladisticMatrix[2:length(CladisticMatrix)], '[[', "Matrix"), ncol))))
  ...
  missing.strings <- apply(matrix(as.numeric(is.na(CladisticMatrix$matrix)), nrow = nrow(CladisticMatrix$matrix)), 2, paste, collapse = "")
  ...
  for(i in 1:length(unique(missing.strings))) {
    ...
    taxa.to.drop <- rownames(CladisticMatrix$matrix)[which(strsplit(unique(missing.strings)[i], "")[[1]] == "1")]
    ...
    if((length(tree$tip.label) - length(taxa.to.drop)) < 3) {
      ...
      if((length(tree$tip.label) - length(taxa.to.drop)) == 1) {
        ...
        for(j in MissingStringCharacters[[i]]) {
          ...
          terminal.states <- CladisticMatrix$matrix[setdiff(tree$tip.label, taxa.to.drop), MissingStringCharacters[[i]]]
      ...
      if((length(tree$tip.label) - length(taxa.to.drop)) == 2) {
        ...
        scored.matrix <- matrix(CladisticMatrix$matrix[-match(taxa.to.drop, rownames(CladisticMatrix$matrix)), MissingStringCharacters[[i]]], nrow = 2, dimnames = list(setdiff(rownames(CladisticMatrix$matrix), taxa.to.drop), MissingStringCharacters[[i]]))
        ...
        # Given it might be easiest to set up all.charmaps as for trees with > 2 taxa maybe this should be removed from the current conditional and placed into the one below?
    ...
    # Can proceed to stochastic character mapping as there are sufficient taxa:
      ...
      for(j in MissingStringCharacters[[i]]) {
        ...
        tip.values <- CladisticMatrix$matrix[pruned.tree$tip.label, j]
        ...
        if(length(sort(unique(unlist(strsplit(tip.values, "&"))))) > 1) {
          ...
          if(CladisticMatrix$ordering[j] == "ord") tip.states <- matrix(0, nrow = ape::Ntip(pruned.tree), ncol = length(sort(unique(unlist(strsplit(tip.values, "&"))))), dimnames = list(pruned.tree$tip.label, range(as.numeric(sort(unique(unlist(strsplit(tip.values, "&"))))))[1]:range(as.numeric(sort(unique(unlist(strsplit(tip.values, "&"))))))[2]))
          
          # Create tip states matrix that will serve as priors for make.simmap (if character is unordered):
          if(CladisticMatrix$ordering[j] == "unord") tip.states <- matrix(0, nrow = ape::Ntip(pruned.tree), ncol = length(sort(unique(unlist(strsplit(tip.values, "&"))))), dimnames = list(pruned.tree$tip.label, sort(unique(unlist(strsplit(tip.values, "&"))))))
