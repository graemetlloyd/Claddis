...
#' TreeEstimateAncestralStates(CladisticMatrix = CladisticMatrix,
#'   Tree = tree)
#' 
#' @export TreeEstimateAncestralStates
TreeEstimateAncestralStates <- function(CladisticMatrix, Tree, EstimateAllNodes = FALSE, EstimateTipValues = FALSE, InapplicablesAsMissing = FALSE, PolymorphismBehaviour = "equalp", UncertaintyBehaviour = "equalp", Threshold = 0.01, AllowAllMissing = FALSE) {
  ...
  MapPrunedTreeNodesToFullTreeNodes <- function(tree, fulltree) {
    ...
    if(write.tree(tree) != write.tree(fulltree)) {
      ...
      Ancestors <- unlist(lapply(Descendants, function(x) Claddis::TreeMostRecentCommonAncestor(descs = x, tree = fulltree)))
