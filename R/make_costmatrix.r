...
#'
#' Under a Dollo assumption the acquisition of a \emph{derived} character state is considered to be sufficiently complex (biologically difficult) that in all probability it only occurred once. Even if lost later in evolution, it is considered that it cannot be reacquired. An example of this is the frequent loss of teeth in dinosaurs (Brocklehurst and Field 2021). Teeth were never reacquired in this group, with tooth-like serrations forming in many birds instead. Modelling such characters with a costmatrix is challenging (indeed, it cannot truly been done with \emph{just} a costmatrix - see below). The way it is typically done (Kitching et al. 1998) is to form the costmatrix like this:
#'
#' \preformatted{    -------------------
#'     |  0  |  1  |  2  |
#' -----------------------
...
#'
#' Sometimes confused with a Dollo character, irreversible or Camin-Sokal (Camin and Sokal 1965) characters, do not allow for any reversals to a prior state once the derived state has been acquired. An irreversible costmatrix might look like this:
#'
#' \preformatted{    -------------------
#'     |  0  |  1  |  2  |
#' -----------------------
...
#' -----------------------------------------------------}
#'
#' Here we see every possible polymorphism is accounted for in the rows and columns and a cost associated with each transition to, from, or between polymorphic states. The assignment of costs can be thought of as the presence-absence "switching" to go from one state to another, with each switch costing 0.5. Thus to go from 0 to 0&1 we are making one "switch", turning state 1 "on", hence the total cost is just 0.5. Similarly, to go from 0 to state 1 we turn "on" state 1 and turn "off" state 0, costing us 0.5 + 0.5 = 1.
#'
...
#'
#' Let's imagine a "good" polymorphic costmatrix is one that assigns costs such that polymorphisms are truly considered optimal solutions when the situation warrants it. We can use a contrived example to show this - the star phylogeny for three tips each with a different state:
#'
#' \preformatted{0   1   2
#'  \  |  /
#'   \ | /
...
#'     X}
#'
#' We might imagine that the polymorphism 0&1&2 ought to be optimal (lowest cost) here as our value for the ancestor X. But if we use the costmatrix from above we find the following costs:
#'
#' \preformatted{---------------------
#' | Ancestral | Total |
#' |   state   |  cost |
...
#' ---------------------}
#'
#' Indeed, we would never favour any polymorphism here. Thus to allow for 0&1&2 to be optimal we must consider other "shapes" for our polymorphism space. This function offers two alternatives: 1. the simplex (the hyperdimensional equivalent of an equilateral triangle), and 2. the hypersphere (the hyperdimensional equivalent of the circle). These two shapes also imply their own distance measures (Euclidean and Great Circle, respectively). Both extend to any dimension as required and both have the critical property of bringing the coordinates of polymorphic combinations closer to the origin, i.e., closer to the single state values, and hence making them more plausible candidates under the parsimony criterion.
#'
...
#' \---/       \---/       \---/       \---/       \---/}
#'
#' Where, again, costs are scaled such that those between single states are the same for non-polymorphic characters. This gives us the following costmatrix:
#'
#' \preformatted{        -------------------------------
#'         |  0  |  1  |  2  | 0&1 | 1&2 |
#' ---------------------------------------
...
#' -----------------------------------------------------}
#'
#' Note that in practical terms the function generates such costmatrices by first establishing an adjacency matrix (which switches are reachable from the current state?) and then converting these to costmatrices using the \link{convert_adjacency_matrix_to_costmatrix} function.
#'
#' \emph{Dollo and irreversible polymorphisms}
#'
#' Dollo and irreversible (Camin-Sokal) characters suffer from the same problem when it comes to polymorphisms, in that polymorphic states confound costmatrix construction. More specifically, for a Dollo character, the state 0&1 would suggest state 1 was acquired previously, but lost in some member(s) of the OTU. For an irreversible character the opposite would be true, and we would assume that state 1 was acquired independently by some member(s) of the OTU and state 0 was acquired previously. This nuance is not really codable in a costmatrix as their construction doesn't allow for the correct information to be conveyed. Thus in practice these options are not allowed by the function. Instead a user may wish to encode the more important part of the polymorphism as a single state (e.g., 0&1 would become 1 for a Dollo character as this would better allow for correct placement of the acqusition of state 1 on the tree). However, this is non-ideal too as it would still lead to the problem of undercounting true changes that ignoring polymorphisms generates in the first place (see Nixon and Davis 1991, who also discuss alternative approaches to dealing with them).
#'
#' \emph{Stratigraphic polymorphisms}
#'
#' The stratigraphic case may seem like it ought to follow the same rule as irreversible characters, but this time polymorphisms have a different logical meaning. Specifically they would encode the presence of a taxon in multiple time units. However, this means that in practice (for estimating tree lengths or ancestral states) the taxon should simply be recoded as the oldest unit. E.g., 1&2&3 should just be recoded as 1. Thus, again, polymorphisms do not belong in a stratigraphc costmatrix, however nothing is really lost by recoding them as a single state.
#'
#' \emph{Polymorphism limits}
#'
#' Even where polymorphisms may be appropriate (i.e., for some ordered and unordered characters) they still represent a major increase to costmatrix size that can cause memory limits to be hit far in advance of costmatrices that exclude them. Specifically, and as shown in our hypercube example, there will be as many possible states as N^2 - 1, where N is the number of single states and the minus one indicates the exclusion of the origin value. Thus the size of costmatrix required grows very quickly:
#'
#' \preformatted{------------------------------
...
#'
#' Note that here any costmatrix (save the stratigraphic kind) is rescaled such that the "base weight" - which can be thought of as the lowest single state to single state transition possible (excluding the diagonal) - is set to 1. However, users may also wish to weight individual characters differently. It may be tempting, then, to simply multiply the values of each costmatrix by that weight for use elsewhere. This is discouraged, however, as: 1. Other Claddis functions and data matrix data structures allow for weights to be encoded elsewhere (this would be at best redundant and at worst incorrect, if weights get applied twice), and 2. This can lead to confounded results in some cases. For example, if weighting a character zero to exclude it from an analysis any ancestral state estimation would become equally likely, meaning (inadvertently) large numbers of redundant calculations get made. In short, do not weight steomatrices directly, but use the weights component of each matrix block in a \code{cladisticMatrix} object instead.
#'
...
#' )
#'
#' # Make an unordered costmatrix including polymorphisms:
#' make_costmatrix(
#'   min_state = 0,
#'   max_state = 2,
#'   character_type = "unordered",
...
#' )
#'
#' # Make an ordered costmatrix including polymorphisms:
#' make_costmatrix(
#'   min_state = 0,
#'   max_state = 2,
#'   character_type = "ordered",
...
#' )
#'
#' # Make an irreversible costmatrix:
#' make_costmatrix(
#'   min_state = 0,
#'   max_state = 2,
...
#' )
#'
#' # Make a stratigraphic costmatrix:
#' make_costmatrix(
#'   min_state = 0,
#'   max_state = 2,
...
make_costmatrix <- function(min_state = 0, max_state, character_type, include_polymorphisms = FALSE, polymorphism_shape, polymorphism_distance, state_ages, dollo_penalty = 999) {
  ...
  if (character_type == "ordered") {
    ...
    if (include_polymorphisms) {
      ...
      if (length(x = all_states) >= 16384) stop("Costmatrix would be too large. Use fewer states.")
      
      # Calculate costmatrix size:
      costmatrix_size <- length(x = all_states)
      
      # Initialise adjacency atrix with zeroes:
      adjacency_matrix <- matrix(data = 0, nrow = costmatrix_size, ncol = costmatrix_size, dimnames = list(all_states, all_states))
      ...
      # Convert adjacency matrix to costmatrix:
      costmatrix <- convert_adjacency_matrix_to_costmatrix(adjacency_matrix = adjacency_matrix)$costmatrix
      
      # Rescale such that adjacent single states are cost one:
      costmatrix <- costmatrix / costmatrix[1, 2]
  ...
  if (character_type == "unordered") {
    ...
    if (include_polymorphisms) {
      ...
      if (polymorphism_distance == "euclidean" || polymorphism_distance == "manhattan") costmatrix <- as.matrix(x = dist(x = t(x = state_presence_matrix), method = polymorphism_distance, diag = TRUE, upper = TRUE))
      ...
      if (polymorphism_distance == "great_circle") {
        ...
        costmatrix <- as.matrix(x = dist(x = t(x = state_presence_matrix), method = "euclidean", diag = TRUE, upper = TRUE))
        
        # Treating distances as the chord length of a circle of radius one transform them to the arc length for the same:
        costmatrix <- 2 * asin(x = costmatrix / 2)
    ...
    # If excluding polymorphisms:
      ...
      costmatrix <- matrix(data = 1, nrow = length(x = single_states), ncol = length(x = single_states), dimnames = list(single_states, single_states))
  ...
  if (character_type == "dollo") {
    ...
    if (include_polymorphisms) {
      ...
      # Stop and warn user:
      stop("If character_type is \"dollo\" then include_polymorphisms cannot be TRUE. This is because there is no safe way to record acquisitions and losses of single states where polymorphisms exist using just a costmatrix approach. Consider recoding as a single state, although note that this will necessarily mean some change(s) are not accounted for.")
  ...
  costmatrix <- list(size = ncol(x = costmatrix), type = character_type, costmatrix = costmatrix, symmetry = symmetry, includes_polymorphisms = include_polymorphisms)
