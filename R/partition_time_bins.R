...
partition_time_bins <- function(n_time_bins, partitions_to_include = "All") {
  ...
  if (any(partitions_to_include == "All")) partitions_to_include <- 0:(length(x = time_bins) - 1)
  ...
  n_partitions <- sum(unlist(x = lapply(X = as.list(x = partitions_to_include), function(x) ncol(combn(n_switches, x)))))
  ...
  while (nchar(split_switches[1]) < (length(x = time_bins) - 1)) split_switches <- apply(expand.grid(split_switches, as.character(0:1)), 1, paste, collapse = "")
