...
#'
#' @param dist.matrix A distance matrix in the format created by \link{MatrixDistances}.
...
#' MatrixTrimmer(distances$DistanceMatrix)
#'
#' @export MatrixTrimmer
MatrixTrimmer <- function(dist.matrix, Tree = NULL) {
    ...
    # Case if distance matrix has gaps:
      ...
      if(length(setdiff(nodes.to.remove, tip.name.nodes.to.remove)) > 0) {
        ...
        for(i in nodes.left.to.remove) {
          ...
          # Find node number:
          originating.node <- TreeMostRecentCommonAncestor(strsplit(i, "%%")[[1]], Tree)
      ...
      Tree <- TreeFixRootTime(original.tree = FullTree, pruned.tree = Tree)
      
      # Find node names:
      node.names <- rownames(dist.matrix)[grep("%%", rownames(dist.matrix))]
      
      # Find node numbers:
      for(j in 1:length(node.names)) names(node.names)[j] <- TreeMostRecentCommonAncestor(strsplit(node.names[j], "%%")[[1]], Tree)
