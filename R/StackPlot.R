...
#' ordination_axes <- matrix(c(x, y), ncol = 2, dimnames = list(as.list(apply(matrix(sample(LETTERS, 8 * 8 * 101, replace = TRUE), nrow = 8 * 101), 1, paste, collapse = "")), NULL))
#'
#' # Assign ages as though taxa range through entire interval (100-0 Ma):
#' ages <- matrix(c(rep(100, 8 * 101), rep(0, 8 * 101)), ncol = 2, dimnames = list(as.list(rownames(ordination_axes)), as.list(c("FAD", "LAD"))))
#'
...
#' ordination_axes <- matrix(rnorm(10000), nrow = 100, dimnames = list(as.list(apply(matrix(sample(LETTERS, 8 * 100, replace = TRUE), nrow = 100), 1, paste, collapse = "")), NULL))
#'
#' # Create random first and last appearance dates for objects:
#' ages <- matrix(as.vector(apply(matrix(runif(200, 0, 100), ncol = 2), 1, sort, decreasing = TRUE)), ncol = 2, byrow=TRUE, dimnames = list(as.list(rownames(ordination_axes)), as.list(c("FAD", "LAD"))))
#'
