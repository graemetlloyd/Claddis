...
#' prunedmatrix <- MatrixPruner(clad.matrix = Gauthier1986, characters2prune = c(11, 53),
...
#'
#' @export MatrixPruner
MatrixPruner <- function(clad.matrix, blocks2prune = c(), characters2prune = c(), taxa2prune = c(), removeinvariant = FALSE) {
  ...
  if(length(setdiff(blocks2prune, 1:(length(clad.matrix) - 1))) > 0) stop("Block numbers specified that are not present in data.")
  
  # Check characters specified exist and stop and warn user if not:
  if(length(setdiff(characters2prune, 1:sum(unlist(lapply(lapply(clad.matrix[2:length(clad.matrix)], '[[', "Matrix"), ncol))))) > 0) stop("Characters specified that are outside the scope of the matrix. Check and retry.")
  
  # Check taxa specified exist and stop and warn user if not:
  if(length(setdiff(taxa2prune, rownames(clad.matrix$Matrix_1$Matrix))) > 0) stop("Taxa specified that are not found in the matrix. Check and retry.")
  ...
  if(!is.null(characters2prune)) {
    ...
    for(i in unique(CharacterBlocks)) {
      ...
      clad.matrix[[(i + 1)]]$Matrix <- clad.matrix[[(i + 1)]]$Matrix[, -ColumnsToDelete, drop = FALSE]
      
      # Remove characters from ordering:
      clad.matrix[[(i + 1)]]$Ordering <- clad.matrix[[(i + 1)]]$Ordering[-ColumnsToDelete]
      
      # Remove characters from weights:
      clad.matrix[[(i + 1)]]$Weights <- clad.matrix[[(i + 1)]]$Weights[-ColumnsToDelete]
      
      # Remove characters from minimum values:
      clad.matrix[[(i + 1)]]$MinVals <- clad.matrix[[(i + 1)]]$MinVals[-ColumnsToDelete]
      
      # Remove characters from maximum values:
      clad.matrix[[(i + 1)]]$MaxVals <- clad.matrix[[(i + 1)]]$MaxVals[-ColumnsToDelete]
  ...
  if(!is.null(taxa2prune)) {
    ...
    for(i in 2:length(clad.matrix)) clad.matrix[[i]]$Matrix <- clad.matrix[[i]]$Matrix[-match(taxa2prune, rownames(clad.matrix[[i]]$Matrix)), , drop = FALSE]
  ...
  if(!is.null(blocks2prune)) {
    ...
    clad.matrix <- clad.matrix[-(blocks2prune + 1)]
    
    # Rename (renumber) remaining matrix blocks:
    names(clad.matrix[2:length(clad.matrix)]) <- paste("Matrix_", 1:(length(clad.matrix) - 1), sep = "")
  ...
  if(removeinvariant) {
    ...
    InvariantsAsList <- lapply(lapply(lapply(lapply(lapply(clad.matrix[2:length(clad.matrix)], '[[', "Matrix"), LengthFinder), unlist), '<', 1), which)
    ...
    if(length(unlist(InvariantsAsList)) > 0) {
      ...
      for(i in 1:length(InvariantsAsList)) {
        ...
        if(length(InvariantsAsList[[i]]) > 0) {
          ...
          # Remove characters from matrix:
          clad.matrix[[(i + 1)]]$Matrix <- clad.matrix[[(i + 1)]]$Matrix[, -InvariantsAsList[[i]], drop = FALSE]
          
          # Remove characters from ordering:
          clad.matrix[[(i + 1)]]$Ordering <- clad.matrix[[(i + 1)]]$Ordering[-InvariantsAsList[[i]]]
          
          # Remove characters from weights:
          clad.matrix[[(i + 1)]]$Weights <- clad.matrix[[(i + 1)]]$Weights[-InvariantsAsList[[i]]]
          
          # Remove characters from minimum values:
          clad.matrix[[(i + 1)]]$MinVals <- clad.matrix[[(i + 1)]]$MinVals[-InvariantsAsList[[i]]]
          
          # Remove characters from maximum values:
          clad.matrix[[(i + 1)]]$MaxVals <- clad.matrix[[(i + 1)]]$MaxVals[-InvariantsAsList[[i]]]
  ...
  NewBlocksToDelete <- which(unlist(lapply(lapply(clad.matrix[2:length(clad.matrix)], '[[', "Matrix"), ncol)) == 0)
  
  # If there are new blocks to prune:
  if(length(NewBlocksToDelete) > 0) {
    
    # Remove blocks to be rpuned:
    clad.matrix <- clad.matrix[-(NewBlocksToDelete + 1)]
