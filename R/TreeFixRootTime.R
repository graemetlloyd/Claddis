...
#'
#' Note that \code{TreeFixRootTime} in the \code{paleotree} package performs the same function, but is not called here to reduce the number of libraries on which \code{Claddis} is dependent. Interested users should also refer to the \code{dropPaleoTip} function in \code{paleotree}.
#'
...
#' pruned.tree <- TreeFixRootTime(tree, pruned.tree)
#' 
#' # Show that the root time is now fixed (19 Ma):
#' pruned.tree$root.time
#' 
#' @export TreeFixRootTime
TreeFixRootTime <- function(original.tree, pruned.tree) {
