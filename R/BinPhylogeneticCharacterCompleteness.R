...
BinPhylogeneticCharacterCompleteness <- function(CladisticMatrix, TimeTree, TimeBins, plot = FALSE, CI = 0.95) {
  ...
  CompleteEdgesInBins <- BinEdgeLengths(TimeTree, TimeBins)$edge.length.in.bin
  ...
  for(i in unique(MissingValues)) {
    ...
    if(!is.na(PrunedTree)[1]) EdgeLengthsInBinsByCharacter[which(MissingValues == i), ] <- matrix(rep(BinEdgeLengths(PrunedTree, TimeBins)$edge.length.in.bin, length(which(MissingValues == i))), ncol = ncol(EdgeLengthsInBinsByCharacter), byrow = TRUE)
