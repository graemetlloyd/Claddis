...
#' @param inapplicable.behaviour What to do with inapplicable characters. Only one option currently ("missing"). See details.
#'
#' @details
#'
...
map_stochastic_changes <- function(cladistic_matrix, time_tree, time_bins, NSimulations = 10, polymorphism.behaviour = "equalp", uncertainty.behaviour = "equalp", inapplicable.behaviour = "missing") {
  ...
  # Check uncertainty.behaviour is correctly formatted or stop and warn user:
  if (length(setdiff(uncertainty.behaviour, c("equalp", "missing", "random"))) > 0) stop("uncertainty.behaviour must be one of \"equalp\", \"missing\" or \"random\".")

  # Check inapplicable.behaviour is correctly formatted or stop and warn user:
  ...
  CharacterList <- lapply(lapply(apply(MatrixBlock, 2, list), unlist), function(x) {
    y <- list()
    y$TipStates <- lapply(apply(matrix(rep(x, times = NSimulations), ncol = NSimulations, dimnames = list(names(x), c())), 2, list), unlist)
    return(y)
  })
  ...
  edit_polymorphisms <- function(x, polymorphism.behaviour, uncertainty.behaviour) {
    ...
    # If polymorphism.behaviour is missing replace all polymorphisms with NAs:
    if (polymorphism.behaviour == "missing") {
      TipStateList <- lapply(TipStateList, function(x) {
        CellsToAlter <- grep("&", x)
        if (length(CellsToAlter) > 0) x[CellsToAlter] <- NA
        return(x)
      })
    }

    # If uncertainty.behaviour is missing replace all uncertainties with NAs:
    if (uncertainty.behaviour == "missing") {
      TipStateList <- lapply(TipStateList, function(x) {
        CellsToAlter <- grep("/", x)
        if (length(CellsToAlter) > 0) x[CellsToAlter] <- NA
        return(x)
    ...
    # If polymorphism.behaviour is random replace all polymorphisms with one included state at random:
    if (polymorphism.behaviour == "random") {
      TipStateList <- lapply(TipStateList, function(x) {
        CellsToAlter <- grep("&", x)
        if (length(CellsToAlter) > 0) for (i in 1:length(CellsToAlter)) x[CellsToAlter[i]] <- sample(strsplit(x[CellsToAlter[i]], split = "&")[[1]], size = 1)
        return(x)
      })
    }

    # If uncertainty.behaviour is random replace all uncertainties with one included state at random:
    ...
    # If uncertainty.behaviour is equalp replace all uncertainties withpolymorphism equivalent (makes grep simpler later):
    if (uncertainty.behaviour == "equalp") TipStateList <- lapply(TipStateList, function(x) gsub("/", "&", x))
  ...
  CharacterList <- lapply(CharacterList, function(x) {
    x$PrunedTipStates <- lapply(x$TipStates, function(y) y[!is.na(y)])
    x
  })
  ...
  CharacterList <- lapply(CharacterList, function(x) {
    Ranges <- range(as.numeric(unlist(strsplit(unlist(x$PrunedTipStates), split = "&"))))
    x$MinVal <- Ranges[1]
    x$MaxVal <- Ranges[2]
    x
  })
  ...
  build_character_map_trees <- function(x) {
    ...
    if (NumberOfTips == 1) {
      ...
      build_single_taxon_map <- function(y, tree) {
        ...
        Output$maps <- lapply(Output$maps, function(z) {
          names(z) <- colnames(y)
          z
        })
  ...
  map_characters_to_full_tree <- function(x) {
    ...
    if (lapply(x$PrunedTipStates, nrow)[[1]] < ape::Ntip(x$FullTree)) {
      ...
      NullMap <- lapply(NullMap, function(z) {
        names(z) <- NA
        z
      })
        ...
        for (i in 1:length(y)) {
          ...
          if (length(MultipleEdgeSingleStateMatches) > 0) {
            ...
            for (j in MultipleEdgeSingleStateMatches) {
              y[[i]]$maps[unname(unlist(x$PrunedToFullTreeEdgeMatches[j]))] <- lapply(y[[i]]$maps[unname(unlist(x$PrunedToFullTreeEdgeMatches[j]))], function(z) {
  ...
  reformat_changes <- function(x) {
    ...
    extract_changes <- function(y) {
      ...
      if (length(FollowingChanges) > 0) {
        ...
        ChangesMatrix <- rbind(ChangesMatrix, cbind(EdgeFromTo[FollowingChanges, 2], FollowingState[FollowingChanges], unlist(lapply(as.list(time_tree$edge[FollowingChanges, 2]), function(z) {
          FollowingEdges <- which(time_tree$edge[, 1] == z)
