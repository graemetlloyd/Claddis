...
match_tree_edges <- function(original_tree, pruned_tree) {
	...
	if (length(removed_taxa) == 0) {
		...
		matching.edges <- as.list(1:nrow(original_tree$edge))
		
		# Create lists of nodes (which will be identical):
		clades <- corresponding.nodes <- c((ape::Ntip(pruned_tree) + 1):(ape::Ntip(pruned_tree) + ape::Nnode(pruned_tree)), 1:ape::Ntip(pruned_tree))
	...
	# If taxa are removed:
		...
		for(i in (ape::Ntip(pruned_tree) + 1):(ape::Ntip(pruned_tree) + ape::Nnode(pruned_tree))) {
			...
			clades <- c(clades, paste(pruned_tree$tip.label[FindDescendants(i, pruned_tree)], collapse = "%%SpLiTtEr%%"))
		...
		for(i in clades) corresponding.nodes <- c(corresponding.nodes, find_mrca(descendant_names = strsplit(i, "%%SpLiTtEr%%")[[1]], tree = original_tree))
		
		# Add tips to node numbers for original_tree:
		corresponding.nodes <- c(corresponding.nodes, match(pruned_tree$tip.label, original_tree$tip.label))
		...
		pruned.edges <- cbind(corresponding.nodes[match(pruned_tree$edge[, 1], clades)], corresponding.nodes[match(pruned_tree$edge[, 2], clades)])
		
		# Find edges that match EXACTLY with the original tree:
		matching.edges <- match(apply(pruned.edges, 1, paste, collapse = "%%"), apply(original_tree$edge, 1, paste, collapse = "%%"))
		...
		if (length(nonmatching.edges) > 0) {
			...
			for(i in 1:nrow(nonmatching.edges)) {
				...
				while(length(sort(match(original_tree$edge[edges, 1], start.node))) == 0) {
		...
		removed.edges <- setdiff(1:nrow(original_tree$edge), as.numeric(unlist(matching.edges)))
		
	}
	
	# Add names to matching edges:
	names(matching.edges) <- 1:nrow(pruned_tree$edge)
