...
#' 
#' # Find matching edges:
#' X <- TreeMatchEdges(original.tree, pruned.tree)
#' 
#' # Show matching edges:
#' X$matching.edges
#' 
...
#' X$removed.edges
#' 
#' @export TreeMatchEdges
TreeMatchEdges <- function(original.tree, pruned.tree) {
	...
	# If taxa are removed:
		...
		for(i in clades) corresponding.nodes <- c(corresponding.nodes, TreeMostRecentCommonAncestor(strsplit(i, "%%SpLiTtEr%%")[[1]], original.tree))
