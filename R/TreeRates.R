...
#' @param EstimateAllNodes Option passed to internal use of \link{TreeEstimateAncestralStates}.
#' @param EstimateTipValues Option passed to internal use of \link{TreeEstimateAncestralStates}.
#' @param InapplicablesAsMissing Option passed to internal use of \link{TreeEstimateAncestralStates}.
#' @param PolymorphismBehaviour Option passed to internal use of \link{TreeEstimateAncestralStates}.
#' @param UncertaintyBehaviour Option passed to internal use of \link{TreeEstimateAncestralStates}.
#' @param Threshold Option passed to internal use of \link{TreeEstimateAncestralStates}.
#'
#' @details
#'
#' \bold{Introduction}
#'
#' Morphological change can be captured by discrete characters and their evolution modelled as occurring along the branches of a phylogenetic tree. This function takes as primary input a character-taxon matrix of discrete characters (in the format imported by \link{ReadMatrixNEXUS}) and a time-scaled phylogenetic tree (in the format of \pkg{paleotree} or \pkg{strap}) and begins by inferring ancestral states at the tree's internal nodes using the \link{TreeEstimateAncestralStates} function. From here changes along individual branches can be estimated (only the minimum number of changes are inferred; see \link{TreeStochasticChanges} for an alternative but unfinished approach) and hence rates can be calculated.
#'
...
#' x <- TreeRates(tree = tree, CladisticMatrix =
...
TreeRates <- function(tree, CladisticMatrix, TimeBins, BranchPartitionsToTest = NULL, CharacterPartitionsToTest = NULL, CladePartitionsToTest = NULL, TimeBinPartitionsToTest = NULL, ChangeTimes = "random", LikelihoodTest = "AIC", Alpha = 0.01, MultipleComparisonCorrection = "BenjaminiHochberg", PolymorphismState = "missing", UncertaintyState = "missing", InapplicableState = "missing", TimeBinApproach = "Lloyd", EnsureAllWeightsAreIntegers = FALSE, EstimateAllNodes = FALSE, EstimateTipValues = FALSE, InapplicablesAsMissing = FALSE, PolymorphismBehaviour = "equalp", UncertaintyBehaviour = "equalp", Threshold = 0.01) {
  ...
  if(!is.null(CladePartitionsToTest)) {
    ...
    CladePartitionsToTest <- lapply(CladePartitionsToTest, lapply, TreeDescendantEdges, tree = tree)
  ...
  TreeNodeAges <- TreeNodeAges(tree)

  # Get branch ages (from and to):
  BranchAges <- unname(cbind(TreeNodeAges[as.character(tree$edge[, 1])], TreeNodeAges[as.character(tree$edge[, 2])]))
  ...
  TreeDescendantEdgesForEachInternalNode <- lapply(as.list(InternalNodeNumbers), TreeDescendantEdges, tree = tree)
  
  # Get ancestral character states:
  AncestralStates <- TreeEstimateAncestralStates(CladisticMatrix = CladisticMatrix, Tree = tree, EstimateAllNodes = EstimateAllNodes, EstimateTipValues = EstimateTipValues, InapplicablesAsMissing = InapplicablesAsMissing, PolymorphismBehaviour = PolymorphismBehaviour, UncertaintyBehaviour = UncertaintyBehaviour, Threshold = Threshold)
  ...
  if(!is.null(BranchPartitionsToTest) || !is.null(CladePartitionsToTest)) {
    ...
    if(!is.null(CladePartitionsToTest)) {
      ...
      CladeRates <- do.call(rbind, lapply(lapply(as.list(ape::Ntip(tree) + c(1:tree$Nnode)), function(x) TreeDescendantEdges(x, tree = tree)), function(y) c(Changes = sum(EdgeChanges[y]), Completeness = sum(EdgeCompleteness[y] * EdgeDurations[y]), Duration = 1)))
...
# Matrix <- Claddis::ReadMatrixNEXUS("~/Documents/Packages/Claddis/LungfishTest/Lloyd_etal_2012a.nex")
