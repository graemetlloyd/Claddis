...
TreeNodeAges <- function(tree) {
  ...
  TreeNodeAges <- vector(mode = "numeric", length = ape::Ntip(tree) + ape::Nnode(tree))
  
  # For each path:
  for(i in 1:length(paths)) {
    
    # Store path lengths from root:
    TreeNodeAges[paths[[i]][1]] <- sum(tree$edge.length[match(paths[[i]][1:(length(paths[[i]]) - 1)], tree$edge[, 2])])
  
  }
  
  # Subtract path lengths from root time:
  TreeNodeAges <- tree$root.time - TreeNodeAges
  
  # Add node numbers:
  names(TreeNodeAges) <- 1:(ape::Ntip(tree) + ape::Nnode(tree))
