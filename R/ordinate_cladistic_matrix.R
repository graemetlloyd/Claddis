...
#' @param time.tree If a phylmorphospace is desired then a tree with root age and branch-lengths must be included.
...
#'
#' @return \item{time.tree}{The tree (if supplied). Note this may be pruned from the input tree by \link{trim_matrix}.}
...
ordinate_cladistic_matrix <- function(cladistic.matrix, distance.metric = "MORD", ged.type = "Wills", distance.transformation = "arcsine_sqrt", distance.polymorphism.behaviour = "min.difference", distance.uncertainty.behaviour = "min.difference", distance.inapplicable.behaviour = "missing", character.dependencies = NULL, alpha = 0.5, correction = "cailliez", time.tree = NULL, estimate.all.nodes = FALSE, estimate.tip.values = FALSE, inapplicables.as.missing = FALSE, ancestral.polymorphism.behaviour = "equalp", ancestral.uncertainty.behaviour = "equalp", threshold = 0.01, allow.all.missing = FALSE) {
  ...
  # Case if a tree is included (and a phylomorphospace is requested):
    ...
    ancestral_values <- estimate_ancestral_states(cladistic.matrix = cladistic.matrix, time.tree = time.tree, estimate.all.nodes = estimate.all.nodes, estimate.tip.values = estimate.tip.values, inapplicables.as.missing = inapplicables.as.missing, polymorphism.behaviour = ancestral.polymorphism.behaviour, uncertainty.behaviour = ancestral.uncertainty.behaviour, threshold = threshold, allow.all.missing = allow.all.missing)
  ...
  # Compile output:
  output <- c(time.tree = list(time.tree), DistMatrix = list(trimmed.distances$DistMatrix), RemovedTaxa = list(trimmed.distances$RemovedTaxa), pcoa.results)
